...
// COMMENT: What is this route doing? Where does it receive a request from?
// (put your response in a comment here) This is put request from the client postgres. app is the representation of the server express client. body is body parser. sql does not understand json so we use request.body to help sql understand the info format being sent over from the client.
/////app.put() is not being called anywhere.  The below function is just setting up a route.  The url is /article/:id  (the article with this unique id). When this route is being hit or when the pg client sends this route to the database, then the database would update the articles table and the authors table with the new info in the array with each array item starting with request.body.  Once the database successfully updated the tables then it would send a reponse back to the client saying 'update complete'. If it cannot update the tables then .catch would console.log the error.  
////The request is to update the authors table, where author_id equals to prime 3 or $3, set the author equal to $1 or prime 1.  $1 corresponds to request.body.author (from the array being sent over from the client request).  Update authorUrl to $2, which corresponds to request.body.authorUrl
/////The request is to update the articles table.  Request.params.id where id comes from the url.  where article_id is equal to $6, set the author_id equal to $1, which corresponds to request.body.author_id......and so on.......
